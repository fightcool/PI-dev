...
本次交付为公开仓库 `fightcool/PI-dev`，部署目标是当前开发服务器 `C202609091757997` 上的 `dev` 用户，不再以 `leika-1`/`deploy` 作为目标。现有私有业务 checkout、未提交修改、数据库、PM2 生产服务及反向代理均不修改。
...
域名 `dev.ftai.cc` 已解析到当前服务器。TLS 反代应将该域名上游指向 `127.0.0.1:8788`，完整转发 Host（`$http_host`），正确升级 `/ws`，保持前端资源与 API 同源，并保留 token 鉴权或可靠的外层身份验证。当前仓库不替用户改 nginx、不生成证书、不开放端口；反代配置必须由服务器操作者单独审核后启用。
...
同一 Unix 用户的 Pi 仍能访问该用户其他文件。当前部署使用 `dev` 非 root 用户；Pi 是高权限远程命令执行界面，不可在未完成认证和反代审核时裸露公网。
