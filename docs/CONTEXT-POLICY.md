...
**先说一个容易踩的统计陷阱**：历史分布本身是「从不压缩」造成的。近 3 天 1165 轮真实请求里 prompt
中位 336k / p90 673k / 峰值 1059k——这正是旧行为（`reserveTokens=16384` → 触发点 1,033,616，几乎永不压缩）
让上下文一路涨上去的结果。用这份分布去数「多少轮会触发压缩」会得出「258400 → 61% 的轮次」，
**那是把旧时代的畸形分布当成了预测**，不可用。

正确的口径是稳态：上下文每轮只增长一点，所以从压缩残留量涨到触发点要很多轮才发生一次。
实测每轮增长（同一会话相邻请求的 prompt 差，7389 次请求 / 13 个会话）：中位 **1k**、p90 2k。
压缩残留按 60k 估（摘要 + `keepRecentTokens` 20k），则有：

| `autoCompactTokenLimit` | 触发点 | 稳态压缩频率（每 `(T−60k)/增长` 轮一次） | 折算每天（实测日均 388–1181 轮） | 峰值 prompt 上限 |
| --- | --- | --- | --- | --- |
| `null`（默认，≈ 窗口 × 95%） | 950k–997k | 每 ~1240 轮 | < 1 次（≈ 不压缩） | ≈ 950k |
| 600000 | 600k | 每 ~690 轮 | ~1–2 次 | 600k |
| 500000 | 500k | 每 ~560 轮 | ~1–2 次 | 500k |
| 400000 | 400k | 每 ~430 轮 | ~1–3 次 | 400k |
| **258400（Codex 同族实跑值）** | 258k | **每 ~250 轮** | **~2–5 次** | **258k** |

上面这张表可以直接复算：`node scripts/maintenance/analyze-context-steady-state.mjs`（只读，输出不含凭据）。

口径限定：每轮增长不均匀（p90 2k、极端单轮 876k——比如一次读进大文件），所以实际频率只会比表中偏高；
但量级是清楚的——**258400 不是「每次都快压缩」，而是每天两三次**，同时把每轮 prompt 从
中位 336k / p90 673k 压到 ~258k 以内。

代价是每次压缩是一次额外的模型往返（用量记为 `source=compaction`），被压掉的旧内容只剩摘要
（摘要 + 最近 `keepRecentTokens` 原文仍保留，所以**长链任务的连续性不丢**，丢的是原文细节）。

**建议**：先用 `258400` 跑一段时间（这是业界对这些模型的实跑值），观察压缩频率与体感；
若觉得压缩过密就调到 400000–600000（改一行、按 mtime 热生效、不用重启）。
