...
| `autoCompactTokenLimit` | 触发点 | 稳态压缩频率（每 `(T−60k)/增长` 轮一次） | 折算每天（日均 388 轮） | 峰值 prompt 上限 |
| --- | --- | --- | --- | --- |
| `null`（默认，≈ 窗口 × 95%） | 950k–997k | 每 ~1240 轮 | < 1 次 | ≈ 950k（等于不压缩） |
| 600000 | 600k | 每 ~690 轮 | ~1 次 | 600k |
| 500000 | 500k | 每 ~560 轮 | ~1 次 | 500k |
| 400000 | 400k | 每 ~430 轮 | ~1 次 | 400k |
| **258400（Codex 同族实跑值）** | 258k | **每 ~250 轮** | **~2–3 次** | **258k** |
