...
本记录描述当前服务器 `C202609091757997`、`dev` 用户上的验证事实，不代表已完成模型授权或公网反代切换。
...
| 本机 smoke | `npm run smoke` 通过：health、cwd、认证、4 个资源和 WebSocket upgrade |
...
- **域名/公网切换**：`dev.ftai.cc` DNS 已指向 `156.226.176.45`，但当前服务器 443 连接被拒绝，尚无 nginx/TLS 反代；公网 smoke 未运行。
...
- **版本迁移**：当前新实例运行 Pi 0.85.1、pi-web-ui 0.70.0；正式切换前不得混用旧实例会话或验证结果。`node-pty` 因系统缺少 make/g++ 无法源码构建，本机使用已验证兼容 Node 22 的 VS Code Server native binary 完成本次运行；全新机器需先安装构建工具。
