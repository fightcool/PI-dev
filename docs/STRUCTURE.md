...
PI-dev 是一个源码仓库，包含开发环境编排、定制 pi-web-ui，以及后续独立的 dev-con 管理面。源码按责任归属；私有配置、会话和上传按实例归属；部署版本按提交归属。
...
| `dev-con/`（后续） | 独立控制平面及 Agent 适配器 | pi-web-ui 的模型请求关键路径 |
...
既有在线XDG目录可继续使用；不要为了目录整齐将凭据或会话迁入Git。其他Agent自己的配置与技能归 dev-con 后续的适配器/资源清单管理，这一轮不扫描复制、同步或删除它们。

同一服务仅由一个主要进程管理器管理。systemd适用于当前Linux工作站，PM2使用单实例fork；Docker Compose使用自身重启策略。会话、PTY和WS状态尚未设计为多进程共享，不能直接启用PM2 cluster实现横向扩容。

## 与后续 dev-con 的衔接

[DEV-CON-PROPOSAL.md](DEV-CON-PROPOSAL.md) 保留独立 `dev-con/`、8791端口与控制平面定位。它后续可调用这里的工程命令和服务入口；不复制用量模块，不在模型请求中转发流量，不自行修改仓库依赖。涉及配置写入和系统操作的授权规则继续由该管理面的适配器负责。

端口约定：8788在线UI、8790发布候选、8791 dev-con、8890开发后端；协议/浏览器测试使用独立空闲端口或完全模拟网络。
