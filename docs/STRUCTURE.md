...
| `vendor/pi-web-ui/server/dev-con/` | 渠道元数据、对话绑定、组合切换命令、账户查询适配器、用量历史（逐请求记录 + 只读聚合 + 保留策略）、系统资源与存储占用只读采集、**渠道端点工具能力探测（网关静默丢弃 tools 时告警）**、**运维诊断快照（仅元数据）、资源告警与渠道失败告警（失败请求的白烧输入 token 统计与提示）** | 第二套模型/密钥事实源、外部原生 Agent 集成、长期计费平台 |
