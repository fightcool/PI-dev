...
| `vendor/pi-web-ui/server/dev-con/` | 渠道元数据、对话绑定、组合切换命令与账户查询适配器（只存引用，不存密钥） | 第二套模型/密钥事实源、外部原生 Agent 集成 |
