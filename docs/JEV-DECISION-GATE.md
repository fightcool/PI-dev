...
### 4.4.1 公共接口声明与判据复核（2026-09-21，PR #88）

#85 / #86 的 API 兼容性判定曾低至 0.09 / 0.11。复核发现，原判据容易把模块导出等同于
公共接口。但这两次 PR 同时修改了文档化安装行为，不能把低分全部解释为模型误判。

`docs/PUBLIC-SURFACE.md` 提供消费者和对外契约的查找范围。它不是穷尽清单，也不是放行名单：
内部函数改动仍须同步全部消费者并保持对外行为兼容；内容寻址复制不能单独证明没有外部依赖。

CI 从 PR 的 `baseSha` 读取声明，提交钩子从 `HEAD`（amend 用第一父提交）读取。
新声明和工作树修改不能授权本次 diff。基线没有声明则省略字段，继续使用通用兼容性判据。
现有 CI 仍执行 PR checkout 的脚本；本改动只隔离声明来源，不声称实现整个门禁的信任隔离。

早期草稿的 50 条样本曾得到 21 approve / 4 block / 25 review，未观察到误放行或误拦。
这不是最终判据的验证，也不能从单次采样推断抖动。此前“10 次误拦 2 次”的说法没有统计依据，已撤回。
最终判据使用同一份 50 条语料，以 `--no-cache --repeat 1` 重跑：50 次真实调用、0 缓存回放、
0 调用失败，费用 $0.004295；当前阈值得到 **18 approve / 6 block / 26 review**，样本标签下
误放行 0、误拦 0。每条只采样一次，不作抖动结论；原语料未补入新声明字段，覆盖的是通用判据。
声明来源隔离由 Git 集成测试覆盖。阈值本次不变。

**三态与 CI 退出码不同**：模型 `review` 表示不确定；当前 CI 采用 block-only 策略，
`review` 仍使 job 退出 0，没有自动创建或强制执行人工审核。因此 block → review 会减少 CI 自动拦截，
不能将其描述为“安全性质保持不变”。样本上的零误放行也不构成总体准确率或安全性证明。
