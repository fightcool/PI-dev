...
### 4.4.1 判据补「对外接口由本仓定义」后的重跑（2026-09-21）

动机：两次 PR（#85 / #86）被门禁判 **block**，`change_preserves_public_api` 分别是 **0.09 / 0.11**。
看判据才发现**判得没错**：原口径写的是「任何导出名都是公共接口」，而两次改动删的都是**同仓内部导出**
（扩展本体是内容寻址复制件，消费者只有同仓单测）。问题不在模型，在**口径**：判据不知道
「本仓的公共接口到底有哪些」。

改法（PR #88）：新增 `docs/PUBLIC-SURFACE.md`（「算公共接口」一节 + 「不算」一节 + 判断方法），
判据的 `focus`/`true.what` 明确「仓库有声明时以声明为准，无外部消费者的内部导出不算公共接口」，
并把声明摘录进 CI 门禁与提交钩子的 state（拿不到声明则字段缺席，**判定照旧**，不跳过门禁）。

同一份 50 条语料重跑（阈值不变：逐命题 `scope .50` / `api .70` / `test .95`）：

| 配置 | 误放行 | 误拦 | 正确放行 | 正确拦下 | 转人工 |
| --- | --- | --- | --- | --- | --- |
| 旧口径 | **0** | 0 | 21 | **8** | **21** |
| 新口径（PR #88） | **0** | 0 | 21 | **4** | **25** |

**代价如实记**：4 条原来是「自动拦下」的现在进**人工复核**（不是放行）—— 软化「任何导出名都算公共接口」
必然让一部分「改导出名 / 改返回类型」的条目从「铁定拦」变成「拿不准，交给人」。安全性质
（**0 误放行**）没变，抖动翻转仍是 0。这个交换是**有意的**：本仓绝大多数提交是同仓内部重构，
按旧口径 10 次里要误拦 2 次，比多几条人工复核贵得多。

`tune` 在同一份语料上给出的进一步建议（**尚未采用**，属独立的阈值决策）：
`api` 的 block 侧从 `0.15` 收到 `0.02` —— 这样 0.07 / 0.11 那类改动会落到**转人工**而不是 block。
采用与否要单独决定：它全局放宽 api 的自动拦截面。
