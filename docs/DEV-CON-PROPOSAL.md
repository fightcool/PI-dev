...
- 当前状态：**开发口径已收敛，功能实现仍暂停**。文档整理完成不代表恢复开发或授权部署。
...
首个交付闭环为P1–P3。跨渠道历史分析、更多监控和独立运维属于后续候选，不自动纳入本期。
...
- 8791与`dev-con/`暂为旧原型/后续运维预留，不是渠道功能的部署前提；当前端口限制不因文档整理而解除。
...
| 阶段 | 交付目标 | 当前状态 |
| --- | --- | --- |
| P0 技术验证与设计收口 | 授权隔离、热切换边界、组合命令/版本、用量事件口径、界面流程 | 待验证；本文档整理不代替技术验证 |
| P1 渠道配置与切换 | 渠道设置、命名凭据、模型映射、对话切换、生效回执和恢复 | 未实现本期闭环 |
| P2 编码界面用量 | 当前渠道、请求/运行/会话Token和费用、可靠渠道归属 | 既有统计可复用，新增口径未验收 |
| P3 余额与配额 | 实际供应商查询、缓存/失败/过期状态和界面整合 | 待确定接口并实现 |
| P4 后续候选 | 跨渠道/项目/时间历史、更多监控、资源展示与必要运维 | 不计入首个交付；按实际需求另排 |

## 9. P0必须给出结论的技术项

这些是实现问题，不是仍需重复确认的产品方向。结论直接记录在本节或链接其测试证据，不能另写竞争性计划。
...
`dev-con/`只读原型及其测试保留，包含独立认证/页面和外部Agent占位。原型的跨标签页注销问题已在汇总提交前修复并通过浏览器回归，不改变暂停开发状态。后续复用、迁移或移除前按本基准审查，不让旧实现反向决定需求，也不自动删除现有工作。
...
合并产品与架构要求，统一配置/切换/统计口径、P0–P3顺序及验收编号；归档旧评估、架构讨论和原型记录；更新根README、目录说明与文档索引。未新增功能代码、依赖、配置迁移或部署动作。后续变更首先更新本文件，所有摘要只链接这里。

整理核验：`git diff --check`通过；19份根/工程文档中的69个本地链接有效；整理前后511个非Markdown文件的内容校验一致；独立只读复核未发现开发口径矛盾或核心要求遗漏。该核验仅证明本次文档整理结果，不代表P0技术验证或功能验收通过。
