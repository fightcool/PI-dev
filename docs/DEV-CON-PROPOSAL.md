...
### 模型目录的时效性（2026-09-18 定）

SDK 的 `ModelRuntime` **只在构造时读一次 `models.json`**，之后仅显式 `refresh()` 才重读；而每个客户端会话（`ClientSession`）有自己的 runtime。“配置改了”与“界面看到的目录”因此可以永久背离：用户新增一个服务商后，旧会话的模型选择器对该渠道显示「该渠道暂无可用的模型」，模板里的 `{baseUrl}` 也解析不出来，而**刷新页面也无效**（同 `clientId` 会复用内存里的同一个 `ClientSession`）。

约定：

1. **目录自愈（`ClientSession.ensureFreshModelCatalog`）**：凡是依赖「服务商/模型表」的操作（`list_models`、账户查询）在开始前都比对 `models.json` 的 mtime+size，变了就先 `refresh({allowNetwork:false})`（见 `server/model-catalog-freshness.ts`）。用户不必重启服务、也不必换浏览器。
...
| 入口 | 内容 |
| --- | --- |
| 现有渠道/模型选择器 | 常用渠道、可用模型、命名凭据、当前有效与待生效选择。**停用的渠道不出现在选择器里**（用户自己关的就不该再占位；“服务商缺失/凭据缺失”必须仍然列出并写清原因 —— 那是需要他去修的问题） |
| 状态栏 | 当前渠道/模型、会话Token及估算费用；支持时显示余额摘要和查询时间 |
| 详情面板 | 请求/本轮/会话用量、渠道分布、账户余额、key配额、异常和手动刷新 |
| 设置页 | 渠道新增/编辑/删除/启用切换、**限定可用模型（白名单，可按服务商 baseUrl 拉取 /models 并合并为候选）**、凭据引用、**账户查询模板（可自配 URL/字段映射，内置预设一键填充）**、**按渠道用量统计**、校验与恢复 |
...
| P3 余额与配额 | 实际供应商查询、缓存/失败/过期状态和界面整合 | 适配器：**用户可配置查询模板**（URL/方法/鉴权/字段映射，内置 DeepSeek/网关/OpenRouter 为预设）+ 有界超时/禁重定向/限频/缓存，失败保留旧值；**已在真实 DeepSeek 官方账号验收**；每个渠道在设置页展示自身用量统计 |
...
| P4 后续候选 | 跨渠道/项目/时间历史、更多监控、资源展示与必要运维 | 已完成七项：①跨渠道/项目/模型/来源/按天用量历史；②系统资源展示；③存储占用明细；④用量历史保留策略；⑤运维诊断快照（只含元数据，可下载 JSON）；⑥资源告警（磁盘/内存/unit 内存越线提示，1 小时冷却，可开关）；⑦失败请求与「白烧」可见 + 渠道失败告警（只认 `stopReason=error`，按渠道/服务商聚合，30 分钟窗 + 次数/失败率双阈值 + 1 小时冷却；用量表新增缓存命中率与失败标注，含旧记录一次性回填脚本与端到端/Chromium 证据，见 `P0-VERIFICATION.md` §14 与 §14.1）。其余监控项按需求另排 |
