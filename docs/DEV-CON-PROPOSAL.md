...
| P3 余额与配额 | 实际供应商查询、缓存/失败/过期状态和界面整合 | 适配器：**DeepSeek 官方 `/user/balance`**、OpenAI 兼容网关、OpenRouter；有界超时/禁重定向/限频/缓存，失败保留旧值，未配置即 unsupported。**已在真实 DeepSeek 官方账号上验收通过**（见 P0 文档 §6）；其余供应商仍待真实账号 |
