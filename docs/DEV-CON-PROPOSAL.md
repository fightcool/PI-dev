...
| P4 后续候选 | 跨渠道/项目/时间历史、更多监控、资源展示与必要运维 | 已完成七项：①跨渠道/项目/模型/来源/按天用量历史；②系统资源展示；③存储占用明细；④用量历史保留策略；⑤运维诊断快照（只含元数据，可下载 JSON）；⑥资源告警（磁盘/内存/unit 内存越线提示，1 小时冷却，可开关）；⑦失败请求与「白烧」可见 + 渠道失败告警（只认 `stopReason=error`，按渠道/服务商聚合，30 分钟窗 + 次数/失败率双阈值 + 1 小时冷却；用量表新增缓存命中率与失败标注，含旧记录一次性回填脚本，见 `P0-VERIFICATION.md` §14）。其余监控项按需求另排 |
