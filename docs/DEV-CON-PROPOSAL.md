...
| P4 后续候选 | 跨渠道/项目/时间历史、更多监控、资源展示与必要运维 | 已完成四项：①跨渠道/项目/模型/来源/按天用量历史；②系统资源展示（CPU/内存/磁盘/应用与 unit 占用）；③存储占用明细（有界只读遍历，标注可清理候选）；④用量历史保留策略（0/7/30/90/365 天）。其余监控与运维仍按需求另排 |
