...
export function validateConfig(config) {
  if (config.root !== ROOT) throw new Error('Checkout moved; rerun configure explicitly.');
  if (config.host !== '127.0.0.1') throw new Error('Only loopback binding is supported.');
  if (!Number.isInteger(config.port) || config.port < 1024 || config.port > 65535 || config.port === 8787) {
    throw new Error('Choose an unprivileged port other than the legacy 8787 port.');
  }
  ...
  for (const field of ['root', 'node', 'dataDir', 'agentDir', 'tokenFile']) {
    if (typeof config[field] !== 'string' || !config[field].startsWith('/') || /[\r\n\0]/.test(config[field])) {
      throw new Error(`Invalid absolute path: ${field}`);
    }
  }
  if (config.dataDir === config.agentDir) throw new Error('Web and agent directories must differ.');
  return config;
}
...
export function runtimeEnv(config, base = process.env) {
  ...
  return {
    ...base,
    PATH: [join(ROOT, '.venv/bin'), join(ROOT, 'node_modules/.bin'), dirname(config.node),
