...
const config = validateConfig(
  existing || {
    ...
    host: "127.0.0.1",
    port: 8788,
    dataDir: join(stateDir, "web"),
    agentDir: join(stateDir, "agent"),
    tokenFile: join(CONFIG_DIR, "token"),
    profile: "lean",
  },
);
if (profileArg) {
...
if (!existsSync(config.tokenFile))
  writeFileSync(config.tokenFile, `${randomBytes(32).toString("hex")}\n`, {
    mode: 0o600,
    flag: "wx",
  });
...
if (!existsSync(settingsPath) || profileArg) {
  ...
  settings.packages = PROFILES[config.profile].map((name) =>
...
console.log(
  `Configured ${CONFIG_FILE}; profile=${config.profile}; cwd=${ROOT}`,
);
console.log(
  "Provider credentials are intentionally not imported. Configure them in the new instance.",
);
