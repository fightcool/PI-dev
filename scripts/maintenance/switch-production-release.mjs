...
const BASE =
  process.env.PI_DEV_DEPLOY_ROOT ?? join(HOME, ".local/share/pi-dev/deploy");
...
const WATCHDOGS = [
  "pi-web-ui-dev-watchdog.timer",
  "pi-web-ui-dev-watchdog.service",
];
const PM2_UNIT = "pi-dev-pm2.service";

const NEW_ID = process.argv[2];
if (!NEW_ID || !/^[a-f0-9]{12}$/.test(NEW_ID))
  throw new Error("Usage: switch-production-release.mjs <releaseId(12hex)>");
...
function pruneAfterSwitch(oldId) {
  try {
    ...
    const result = pruneReleases(options, {
      keep: KEEP_RELEASES,
      apply: true,
      protect: [oldId],
    });
    log(
      `prune: 删除 ${result.removed.length} 个历史版本（保留 current + 回滚点 ${oldId} + keep=${KEEP_RELEASES}）${result.removed.length ? `：${result.removed.join(", ")}` : ""}`,
    );
    const stores = pruneObsoleteStores(
      join(BASE, "shared"),
      result.retained,
      join(BASE, "releases"),
      { apply: true },
...
const phase = (name, extra = {}) => {
  try {
    ...
    writeFileSync(
      STATUS_FILE,
      JSON.stringify(
        { phase: name, at: new Date().toISOString(), target: NEW_ID, ...extra },
        null,
        2,
      ) + "\n",
      { mode: 0o600 },
...
const manager = (action) =>
  execFileSync(process.execPath, [join(DEV_ROOT, "scripts/pm2.mjs"), action], {
    encoding: "utf8",
    timeout: 120_000,
    stdio: ["ignore", "pipe", "pipe"],
  }).trim();
...
const { waitForHealth } = await import(
  join(DEV_ROOT, "scripts/lifecycle/health.mjs")
);
const config = validateConfig(JSON.parse(readFileSync(CONFIG_FILE, "utf8")));
const { sendControlCommand } = await import(
  join(DEV_ROOT, "vendor/pi-web-ui/dist/server/control-socket.js")
);
...
const OLD_ID = basename(
  execFileSync("readlink", ["-f", currentLink], { encoding: "utf8" }).trim(),
);
const newPath = join(BASE, "releases", NEW_ID);
if (!existsSync(join(newPath, "release-source.json")))
  throw new Error(`release ${NEW_ID} is missing release-source.json`);
...
async function verifyStrict(vendorRoot) {
  ...
  if (!health.ok || health.headers.has("set-cookie") || !page.ok)
    throw new Error("public ingress health failed");
  const html = await page.text();
  const entry = readFileSync(
    join(vendorRoot, "web/dist/index.html"),
    "utf8",
  ).match(/src="([^"]+\.js)"/)?.[1];
  if (!entry || !html.includes(entry))
    throw new Error("public ingress is not serving this release's frontend");
  ...
  await new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      socket.terminate();
      reject(new Error("WS probe timed out"));
    }, 8_000);
    socket.once("unexpected-response", (req, res) => {
      ...
      res.statusCode === 401
        ? resolve()
        : reject(new Error(`unexpected WS status ${res.statusCode}`));
    });
    socket.once("open", () => {
      clearTimeout(timer);
      socket.close();
      reject(new Error("anonymous WebSocket accepted"));
    });
    socket.once("error", () => {
      clearTimeout(timer);
      reject(new Error("WS probe failed"));
    });
  });
  return entry;
}
...
try {
  ...
  if (!initial?.ok) {
    ...
    log(
      "WARNING: control socket unavailable and SWITCH_ALLOW_NO_SOCKET=1 → skipping the drain step (running conversations will be interrupted)",
    );
  ...
  if (OLD_ID === NEW_ID) {
    const info = JSON.parse(
      readFileSync(
        join(newPath, "vendor/pi-web-ui/dist/build-info.json"),
        "utf8",
      ),
    );
    const source = JSON.parse(
      readFileSync(join(newPath, "release-source.json"), "utf8"),
    );
    if (info.commit === source.commit) {
      log(
        `already deployed: current=${NEW_ID} pid=${initial.pid} commit=${info.commit.slice(0, 12)} (no action taken)`,
      );
      phase("already_deployed", { pid: initial.pid, commit: info.commit });
      process.exitCode = 0;
    } else {
      log(
        `current already points at ${NEW_ID} but provenance differs; continuing with a normal switch`,
      );
    }
  }
  log(
    initial?.ok
      ? `serving pid=${initial.pid} quiesced=${initial.quiesced} active=${initial.activeConversations} pending=${initial.pendingMessages} drainable=${initial.drainableMessages ?? "?"} orphaned=${initial.orphanedMessages ?? "?"}`
      : "serving instance did not answer the control socket",
  );
  log(`current=${OLD_ID} → target=${NEW_ID}`);
  phase("quiesce");
  if (initial?.ok && !(await control("quiesce"))?.ok)
  ...
  for (; quiesced;) {
    ...
    if (Date.now() > deadline)
      throw new Error(
        `active work did not drain within ${WAIT_MIN} minutes（${drainVerdict.detail}）`,
      );
    await delay(2_000);
  }
  ...
  for (const unit of WATCHDOGS)
    if (systemctl(["is-enabled", unit], true) === "enabled")
      systemctl(["disable", unit], true);
  if (isActive(LEGACY)) systemctl(["stop", LEGACY]);
  if (systemctl(["is-enabled", LEGACY], true) === "enabled")
  ...
  if (isActive(PM2_UNIT))
    throw new Error(`${PM2_UNIT} did not stop before the release swap`);
  ...
  await waitForHealth(
    { ...config, workspaceDir: config.workspaceDir ?? config.root },
    { pid: s2.pid, timeout: 30_000 },
  );
  const info = JSON.parse(
    readFileSync(
      join(newPath, "vendor/pi-web-ui/dist/build-info.json"),
  ...
  if (
    info.commit !==
    JSON.parse(readFileSync(join(newPath, "release-source.json"), "utf8"))
      .commit
  )
  ...
  quiesced = false;
  log(
    `DEPLOYED ${OLD_ID} → ${NEW_ID} pid=${s2.pid} commit=${info.commit.slice(0, 12)} protocol=${info.protocolVersion} entry=${entry}`,
  );
  phase("deployed", {
    pid: s2.pid,
    commit: info.commit,
   ...
   *   就会出现「本体问新内核要旧子命令」这类版本错位 —— 而部署切换正是纠正它的唯一时刻。
  ...
  try {
    const out = execFileSync(
      process.execPath,
      [join(newPath, "scripts", "install-jev-hook.mjs"), "install"],
      {
        encoding: "utf8",
    ...
    const installed = out
      .trim()
      .split("\n")
      .filter((line) => line.includes("已安装"));
    log(
      `extensions synced from ${NEW_ID} into ${config.agentDir}（${installed.length} 项）`,
    );
  } catch (error) {
    log(
      `WARNING: 扩展本体同步失败（不影响站点，但扩展可能与内核版本不一致）：${error.message}`,
    );
  }
  ...
  if (touchedManagers) {
    ...
    try {
      ...
      try {
        ...
        log(
          `PM2 start failed (${pm2Error.message}); falling back to the legacy unit`,
        );
        ...
        phase("rolled_back", {
          to: OLD_ID,
          manager: "legacy",
          error: String(pm2Error.message).slice(0, 200),
        });
      }
      if (quiesced) {
      ...
      log(
        `RECOVERY FAILED (manual intervention required): ${recovery.message}`,
      );
      phase("recovery_failed", {
        error: String(recovery.message).slice(0, 300),
      });
    }
  } else if (quiesced) {
    await control("unquiesce").catch(() => {});
  ...
  log(
    `current → ${execFileSync("readlink", ["-f", currentLink], { encoding: "utf8" }).trim()}`,
  );
  log(
    `units: ${PM2_UNIT}=${systemctl(["is-active", PM2_UNIT], true)} ${LEGACY}=${systemctl(["is-active", LEGACY], true)} (legacy enabled=${systemctl(["is-enabled", LEGACY], true)})`,
  );
}
