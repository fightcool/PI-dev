...
export class DshClientSession {
	...
	async queryUsageHistory(reqId: number, query: { groupBy: "provider" | "project" | "model" | "source" | "day" }): Promise<void> {
		this.emit({
			...
			to: null,
			rows: [],
			totals: { requests: 0, input: 0, output: 0, cacheRead: 0, cacheWrite: 0, total: 0, cost: 0, unpricedRequests: 0, unreportedRequests: 0, failedRequests: 0, wastedInput: 0, cacheHitRate: null },
			scanned: 0,
			skipped: 0,
			truncated: false,
		});
