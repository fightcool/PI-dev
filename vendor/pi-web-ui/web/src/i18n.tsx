...
export const zh = {
	...
	chromeAutoCollapseDesc:
		"开启：输出中或向上翻阅会话时自动收起目标条与输入工具条，把竖向空间留给正文（滑到最底部、点输入框或点⌃展开即恢复）；关闭：保持原样",
	...
	gatewayChip: "网关",
	gatewayChipTip: "点开用量明细并立即重新查询（网关账单是异步统计的，数字天然滞后几秒）",
	gatewayUsageTitle: "网关自报用量",
	gatewayUsageUsed: "已用",
	gatewayUsageLimit: "额度",
	gatewayUsageRemaining: "剩余",
	gatewayUsageNotReported: "未报告",
	gatewayUsageScopeCumulative: "网关报告的是累计值（这台部署不按时间窗口细分）",
	gatewayUsageScopeWindow: "统计窗口：近 {days} 天",
	gatewayUsageUnlimited: "网关未设额度上限（返回的是占位值），只能看已用",
	gatewayUsageCheckedAt: "读取于",
	gatewayUsageStale: "数字可能已滞后",
	gatewayUsageRefresh: "刷新",
	gatewayUsageBusy: "查询中…",
	gatewayUsageFailed: "查询网关用量失败",
	gatewayUsageNone: "还没有读取过网关用量",
	gatewayUsageUnsupported: "该服务商没有账单接口（直连上游，不是网关）：读不到用量",
	gatewayUsageMismatch: "这份读数来自 {provider}，不是当前模型的服务商",
	gatewayUsageHint: "这是网关自己的账单数字，与上面的本地按 token 估算**不是**同一回事，两者不能相加。",
	settingsGateway: "网关",
	settingsGatewayKeyEntry: "去「网关」里配置密钥",
	gatewayModelsEnabled: "已启用 {on} / 共 {total}",
	gatewayModelsToggleHint: "点一下模型即可开/关：关掉的模型不会出现在模型选择器里（只是收起来，配置与历史记录都不动）。",
	gatewayModelEnable: "点击启用（重新出现在选择器里）",
	gatewayModelDisable: "点击关闭（从选择器里收起来）",
	...
	gatewayLoadFailed: "读取网关配置失败",
	gatewayBaseUrl: "接口地址（baseUrl）",
	gatewayBaseUrlPh: "https://api.example.com/v1",
	gatewayProtocol: "接口协议",
	gatewayProtocolHint: "网关支持哪种协议由它自己决定；换错协议的表现是请求 404/400，而不是「模型不存在」。",
	gatewayApiKey: "API 密钥",
	gatewayApiKeyPh: "sk-…",
	gatewayApiKeyKeep: "已保存，留空 = 不修改",
	gatewayApiKeySaved: "已保存密钥（正文不回显、不出服务端）",
	gatewayApiKeyClear: "清除已保存的密钥",
	gatewaySave: "保存网关配置",
	gatewaySaving: "保存中…",
	gatewaySaved: "已保存",
	gatewaySaveFailed: "保存失败：{error}",
	gatewayModels: "模型清单",
	gatewayModelsCount: "{n} 个",
	gatewayModelsEmpty: "还没有模型：先填地址与密钥，再从网关读取一次。",
	gatewayModelsProbe: "从网关读取模型清单",
	...
	gatewayDupRemove: "删除",
	gatewayDupRemoveConfirm: "删除 {id}？它的模型注册会一并移除（历史用量记录保留）。",
	usageColProvider: "服务商",
	usageUnattributed: "未归属",
	usageColSource: "来源",
	usageColChannel: "渠道",
	usageColModel: "模型",
	...
	upstreamErrorQuota: "该渠道的网关账户额度不足（网关报告剩余 {remaining}）。请为这个账户充值，或切换到其他渠道。",
	upstreamErrorQuotaNoRemaining: "该渠道的网关账户额度不足。请为这个账户充值，或切换到其他渠道。",
	upstreamErrorDetail: "详情",
	upstreamErrorDetailHide: "收起详情",

	/* Jev 决策门禁（设置面板分区；服务端 jev_status / jev_config_save / jev_probe）。 */
	settingsJev: "Jev 决策门禁",
	settingsJevDesc:
		"编码前先问 Jev 一次：放行 / 拦下 / 转人工。这里配置它的密钥引用、调用渠道与阈值，并直接查看它最近的运行数据与自检结果。",
	settingsJevEnable: "启用 Jev 决策门禁",
	settingsJevEnableHint: "关闭后不再向 Jev 询问决策（请求全部直接放行）；已产生的统计保留。",
	settingsJevStatusMissing: "尚未读取到运行状态（点「重新载入」重试）",
	settingsJevStatusFailed: "读取运行状态失败",
	settingsJevReload: "重新载入",
	settingsJevCredentialTitle: "API KEY（按名称引用）",
	settingsJevCredentialHint: "这里只保存密钥名称的引用：密钥正文只在服务端解析，界面不回显、也不上行。",
	settingsJevCredentialProvider: "服务商",
	...
	settingsJevApproveAt: "approve 阈值（≥ 直接放行）",
	settingsJevBlockAt: "block 阈值（≤ 直接拦下）",
	settingsJevThresholdsHint:
		"Jev 对同一输入的输出概率抖动可达 ~0.08：两个阈值之间是「转人工」的空白带。收窄到 0.5 附近会让同一请求时通时拦，请留出这个带宽（例如 0.35 / 0.65）。",
	settingsJevThresholdsInvalid: "阈值不合法：需要 0 ≤ block 阈值 < approve 阈值 ≤ 1，且中间留出抖动带宽。",
	settingsJevLimitsTitle: "服务端限制（只读）",
	settingsJevTimeout: "单次超时",
	settingsJevCacheTtl: "结果缓存",
	settingsJevMinInterval: "最小间隔",
	settingsJevBalanceTitle: "余额（复用渠道账户查询）",
	settingsJevBalanceNoChannel: "没有可用于余额查询的渠道：请在「渠道」里为当前服务商配一个带账户查询的渠道。",
	settingsJevRuntimeTitle: "运行状态",
	settingsJevRuntimeEmpty: "还没有调用记录",
	settingsJevTotalCalls: "总调用",
	settingsJevOutcomeApprove: "放行",
	settingsJevOutcomeBlock: "拦下",
	settingsJevOutcomeReview: "转人工",
	settingsJevFailures: "失败",
	settingsJevCacheHits: "缓存命中",
	settingsJevAvgElapsed: "平均耗时",
	settingsJevLastError: "最近一次错误",
	settingsJevPropositionsTitle: "可用命题",
	settingsJevPropositionsHint: "每条命题是它能判的一件事；展开可看判定说明与成立/不成立的口径。",
	settingsJevPropositionsEmpty: "服务端没有返回命题清单",
	settingsJevPropositionInstructions: "判定说明",
	settingsJevPropositionTrue: "成立",
	settingsJevPropositionFalse: "不成立",
	settingsJevProbe: "测试连接",
	settingsJevProbeRunning: "正在测试…",
	settingsJevProbeHint:
		"「测试连接」发一次真实决策请求（会计入上面的调用统计）：能返回结果就说明密钥、端点、模型都对；保存配置前也可以先测。",
	settingsJevProbeOk: "连接正常",
	settingsJevProbeDecision: "判定结果",
	settingsJevSave: "保存配置",
	settingsJevSaved: "配置已生效",
	settingsJevRejected: "服务端拒绝了这次配置",
	settingsJevProbeChecks: "判定项分数",
	settingsJevProbeAudit: "本次调用",
	settingsJevCacheMiss: "缓存未命中",
	settingsJevCredentialKeyPlaceholder: "请选择密钥名",
	settingsJevCredentialUnset: "未绑定密钥：门禁无法调用 Jev（可以保存，但自检与判定都会失败）。",
	settingsJevCredentialStale: "该密钥名在当前服务商下已不存在（可能已删除或改名）：不改就会在调用时报错。",
