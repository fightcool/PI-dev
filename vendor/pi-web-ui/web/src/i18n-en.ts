...
export const en: EnDictionary = {
	...
	channelProviderKeysEntry: "Built-in providers & keys",
	channelModelMetaTitle: "Model details",
	channelModelMetaHint:
		"Edits the model catalog (models.json): display name, context window, max output, reasoning/vision support. Hand-tuned fields such as pricing and thinking-level maps are preserved.",
	channelModelMetaEmpty: "This provider has no models in the catalog (fetch or add models in the channel first).",
	channelModelMetaName: "Display name",
	channelModelMetaContext: "Context window",
	channelModelMetaMaxTokens: "Max output",
	channelModelMetaUnset: "unset",
	...
	channelAccountRemaining: "Remaining",
	...
	channelAccountModeTemplate: "JSON template (write the endpoint and field mapping yourself)",
	channelAccountKindNone: "No query",
	channelAccountKindTemplate: "JSON template",
	channelAccountKindGateway: "Gateway probe",
	channelAccountKindOther: "Custom ({kind})",
	channelAccountPreset: "Preset",
	channelAccountPresetPh: "Pick a preset to fill the template",
	channelAccountJsonLabel: "Config template (JSON)",
	channelAccountJsonError: "Invalid JSON: {err}",
	channelAccountGatewayNote:
		"Built-in probe adapter: it uses the channel's API key to probe the billing/quota endpoints automatically, so the textarea can stay empty (fill unit / topupUrl only if you want to pin a unit or a top-up link).",
	channelAccountPlaceholders:
		"Paste a template or pick a preset, then edit. Placeholders use single braces {baseUrl} / {apiKey} (not double braces); map values are JSON paths supporting a.b[0].c and x ?? y fallback chains; map.extra supports ${path} interpolation.",
	channelAccountTemplateHint:
		'A preset is just the starting JSON — keep editing after picking one. Account queries only cover balance/quota (bounded timeout, rate limit, last result kept on failure); save first, then verify with the inline "Query account".',
	channelAccountUrlRequired: "A JSON template needs request.url (the endpoint URL)",
	channelTemplateOverwrite: "Saving replaces this channel's stored account-query config with this JSON wholesale.",
	...
	channelInUse: "In use",
	channelInUseTip: "This conversation is using this channel",
	channelAccountNotQueried: "Balance not queried",
