...
const require = createRequire(join(ROOT, 'node_modules/pi-web-ui/package.json'));
const pty = require('node-pty');

test('native terminal resolves project Python and cwd', async () => {
  const terminal = pty.spawn('python', ['-c',
    'import os,sys; print(os.getcwd()); print(sys.prefix); print(sys.version_info[:2])'], {
    name: 'xterm-color', cols: 80, rows: 24, cwd: ROOT, env: runtimeEnv(loadConfig()),
  });
  let output = '';
